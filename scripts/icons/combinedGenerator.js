...
// Preserve the old Headway icon names generated by move_to_native.js by first
// converting camelCase to snake_case (e.g. `arrowLeft` -> `arrow_left`).
function sanitizeName(name) {
  const sanitized = camelToSnake(name)
    .replace(/[\s-]+/g, '_')        // spaces and dashes to underscores
    .replace(/[^a-zA-Z0-9_]/g, '')  // remove special chars

  return /^\d/.test(sanitized) ? `i_${sanitized}` : sanitized  // prefix with i_ if starts with digit
}
...
const getSvgIcon = (
  ...
  const finalFill = (tintColor && (meta.svgType === 'fill' || meta.svgType === 'both')) ? tintColor : undefined
  const finalStroke = (tintColor && (meta.svgType === 'stroke' || meta.svgType === 'both')) ? (stroke || tintColor) : stroke
